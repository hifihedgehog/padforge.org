...
| Transport and lifecycle | `LinkServer.cs`, `LinkConnection.cs`, `LinkConnectionLifetime.cs`, `LinkAdmissionCoordinator.cs`, `OwnedLinkCallbacks.cs`, `LinkSession.cs`, `TcpControlChannel.cs`, `UdpControlChannel.cs`, `AntiReplayWindow.cs` |
...
| Remote assignments | `LinkAssignments.cs`, `PadForge.App/Services/RemoteAssignmentService.cs`, `PadForge.App/Views/RemoteAssignmentsDialog.xaml.cs` |
...
The same field also accepts a connection code, which is the internet lane (#294). `LinkCode.LooksLikeCode` and `LinkCode.TryParseSelfContained` sort the two apart before anything else: an entry with a dot or a colon is never a code, and a bare name that does not parse as a self-contained code goes to the address path.
...
It holds a `List<LinkPeerConnection>`, so one PC can be linked to many peers at once. The mesh is many-to-many: pairing is pairwise (one handshake and one six-digit code per pair), but `PushLocalFrame` fans a shared device's input out to every consuming peer, and `PushOutputEffect` and `PushAudio` send the reverse frame through the connection that advertised the device, checked against that device's link slot and id, so each device's feedback routes back to the right owner. There is no two-PC cap.

`LinkConnection` runs the handshake (initiator or responder) and returns the data key, and `LinkServer` builds the connection's `LinkSession` from it. `LinkSession` seals every datagram: a 14-byte authenticated header (message type and epoch, slot, a 32-bit sequence, a microsecond timestamp) plus ChaCha20-Poly1305 ciphertext and a 16-byte tag. The sequence doubles as the nonce counter, and the two directions use disjoint salts so both peers share one key without a nonce collision. Receive is verify-then-window: check the tag, then run `AntiReplayWindow`, so a forged sequence never advances the replay state. The session refuses to seal once the 32-bit sequence would wrap, so going on takes a fresh handshake.

`LinkMessageType` values: `Input` (an absolute device-state frame, owner to consumer), `Keepalive`, `Output` (a tagged effect frame, consumer to owner), `Audio` (a speaker PCM block), `DeviceList` (the owner's exposed-device set, resent every two seconds for hot-plug sync), `SourceDemand` (a consumer-to-owner report that a live mapping on the consumer polls a demand-gated source on this device, #241, see below), `PathOffer` (either side's current candidate endpoints, sent over a relayed session only so a relay-started session can upgrade to a direct path, #294), and `Assignments` (a device owner's queries and edits of which virtual controllers its shared devices feed on the peer, which the peer applies only when its trust entry for the owner allows remote assignments). A legacy `Haptic` type is superseded by `Output` and unused. The initial device lists are also exchanged during the handshake under a separate control key. Each V1 device entry carries the stable slot, a stable id, the name, VID and PID, capability counts, a capability bitmask, and the input-device type. The capability bitmask packs eight flags into one byte: `HasRumble` (value 1), `HasRumbleTriggers` (2), `HasGyro` (4), `HasAccel` (8), `HasTouchpad` (16), `HasHaptic` (32) so a remote wheel or FFB stick's force-feedback pipeline runs, `Online` (64) so active/inactive propagates, and `HasAccelAux` (128) so an aux/left-side accelerometer (Nunchuk, left Joy-Con, #199) exposes. `HasAccelAux` (value 128) exhausts the caps byte, so the capabilities that arrived later ride a second capability byte in the `0xE4` device-list tail below rather than another bit.
...
Objects are forwarded only for device types whose names the consumer cannot synthesize locally: Consumer Control (per-session dynamic slots), NFC (tags named from the owner's registry), Microphone (#317, buttons named from the owner's phrase registry), Handheld Buttons (#343, every button a name from the owner's learned set), Head Tracker, Tablet, VR Controller, and Logitech G-Keys (key names from the owner's Logitech software). A gamepad, joystick, keyboard, mouse, or MIDI shape is synthesized identically on the consumer, so shipping its objects would spend budget for nothing. The push travels as one UDP datagram into a 4 KB receive buffer on old peers, so the encoder caps the payload at 3800 bytes and gives no one device more than half of it. Each device ships the named inputs that fit its share, so one large row cannot starve the rows after it. Once the budget runs out, the remaining devices get empty object sections and the consumer falls back to synthesized names for them, rather than the whole list becoming undeliverable. The budget targets old peers only: the current receive loop (`LinkServer.UdpLoopAsync`) takes datagrams into a 64 KB buffer, the UDP maximum, so an oversized frame can no longer vanish into a receive error.

Seven more tails follow the `0xE2` extension, each marked by its own magic byte and appended in order:
...
| `0xE4` (`DeviceListExtV3Magic`) | Second capability byte | The post-exhaustion capability flags: `HasNfcReader` (value 1, #241), `HasGyroAux` (2, #252), and `HasExtraGenericAxes` (4, #193 over the wire), plus the touchpad's pressure and click answers (8 marks `TouchpadPressureSupported` known and 16 carries it, 32 marks `TouchpadClickSupported` known and 64 carries it). |
...
| `0xE9` (`DeviceListExtV8Magic`) | Explicit-empty flags | One byte per device: bit 0 set when the supported button set is empty on purpose, bit 1 when the supported axis set is. The `0xE7` and `0xE8` masks spell dense and empty as the same single 0 byte, so without this flag a pad the owner knows has no sticks arrived advertising all six. |
...
**Codes.** `LinkCode` uses Crockford base32 (no I, L, O, U) so a code reads aloud unambiguously, and it comes in two shapes. A short code is `ShortCodeLength` (8) characters and is an opaque rendezvous key. Only tests mint one, and the app mints and accepts only the long form. A long self-contained code packs 28 bytes (version, flags, public endpoint, private endpoint, an 8-byte fingerprint prefix, an expiry in minutes, and the NAT kind plus delta) and is emitted grouped in fives. The code is not a secret and authenticates nobody. The fingerprint prefix only buys an early mismatch abort, and the SAS plus Ed25519 handshake still gates trust. `TwoWayPunchNonce` derives the shared 16-byte punch nonce from both fingerprint prefixes in sorted order, and `IsHandshakeInitiator` gives the lexicographically smaller fingerprint the initiator role, so the two sides agree with no extra exchange.

**Predicting ports.** `PortPredictor.BuildSprayTargets` puts the peer's raw advertised endpoints first (a cone peer or a shared LAN connects with no prediction), then, for a sequential-symmetric peer, a forward window of `DefaultWindowSteps` (512) delta-steps on its public IP, each surrounded by a `DefaultNeighborhood` (2) radius to absorb jitter from other subscribers on the same CGNAT pool. De-duplicated on the full address string, since `MapToIPv4` on a real IPv6 candidate keeps only four bytes and silently dropped distinct v6 candidates.

**Punching.** `HolePuncher` sprays authenticated probes at every candidate simultaneously so both NATs open. A probe is `ProbeLen` (25) bytes: `TagPing` `0xC2` or `TagPong` `0xC3`, then `PrefixLen` (8) bytes of the sender's fingerprint, then the 16-byte nonce. Carrying the sender prefix is what lets an unsolicited receiver derive the shared nonce and answer without the user clicking anything. `MaxProbeBudget` caps a call at 20,000 probes. `PunchAsync` drops this PC's own endpoints and sprays private (LAN) addresses first, then the rest in the order given. `PunchedConnection` then runs the unmodified handshake over the won path, both sides spraying and listening, and yields the same `LinkConnectionResult` the TCP path produces.

**The reliable channel over UDP.** `UdpControlChannel` is `ILinkControlChannel` over datagrams: a stop-and-wait-per-message ARQ, `TagData` `0xC0` and `TagAck` `0xC1`, each followed by a big-endian u32 channel id and u32 sequence. On a punched path the channel id derives from the punch nonce (the relay lanes derive it from the code or from the pair's capability), so a delayed datagram from a previous attempt on the same endpoint pair can never occupy a fresh channel's sequence space. These tags demux cleanly from sealed data, whose first byte is `(type << 4) | epoch` with type 1 through 9. A send that faults or is canceled poisons the channel rather than leaving the peer's expected sequence unknowable.
...
**Presence for paired peers.** `Dht.PresenceRecord` and `Dht.PresenceService` keep a paired peer findable after either side moves. The slot's salt and the record's encryption key derive from the pairwise 32-byte rendezvous capability established at pairing and stored on `PeerTrust`, never from the Ed25519 public key, because BEP 44 publishes that key in the clear to every storage node. The record is signed with the PC's identity key, so the slot's DHT target is `SHA1(identity public key || salt)`. One slot per pair per direction. `Dht.DhtPresenceStore` does the iterative Kademlia lookup (`K` 8, `Alpha` 8, `MaxRounds` 20, read-only per BEP 43) against `router.bittorrent.com`, `dht.transmissionbt.com`, `router.utorrent.com` (all port 6881) and `dht.libtorrent.org:25401`. A publish counts as healthy at 8 or more acks. Each record carries a one-hour expiry, inside the roughly 2-hour BEP 44 storage expiry. `Dht.RemoteLinkInternetService` is the policy loop that ties it together, driven from `InputService` on a 60-second maintain tick, and every pass republishes presence for each paired peer that has a rendezvous capability. `PresenceService.NextRepublishDelay` defines a 30-to-45-minute jittered cadence that only tests call.

**Relay fallback.** Where both sides sit behind symmetric or carrier-grade NAT there is no direct path at all, so `IrohRelayClient` speaks the open iroh relay protocol to the free n0 relays (`use1-1`, `usw1-1`, `euc1-1`, `aps1-1` at `.relay.n0.iroh.link`). WebSocket to `wss://host/relay` with subprotocol `iroh-relay-v2`, then a challenge and response handshake signing `blake3::derive_key("iroh-relay handshake v1 challenge signature", challenge)` with the client's Ed25519 relay key: ephemeral for an outgoing dial, derived from the host's code for the code listener, and derived from the PC's identity public key for the identity listener. A peer is addressed by that key, so no IP, port, or NAT is involved. `LinkServer` layers a HELLO exchange on top (`TagRelayHello` `0xC4`, `TagRelayHelloAck` `0xC5`) so a caller knows someone is listening before it starts the handshake. PadForge hosts nothing, and the relay sees no secret: the handshake crosses it as public keys, nonces, and signatures, and everything after the handshake is sealed by `LinkSession` AEAD.
...
`PeerTrust` persists per peer as XML attributes: the pinned Ed25519 public key (an identity, not a secret), the name, the self-asserted host name, the pairing time, a reconnect-enabled flag (default true), the gamepad-only flag, `AllowRemoteAssignments` (default false, this PC's consent for that peer to assign its shared devices here), and `RendezvousCapabilityBase64` (the 32-byte pairwise DHT rendezvous capability from #294, empty for a peer paired before the internet lane existed until its next reconnect fills it). The fingerprint is the SHA-256 of the public key.
...
A 16-bit present-block bitmask (`Block`, a `[Flags]` enum in `CustomInputStateCodec`) follows the one-byte codec `Version` at the head of the payload. A block appears only when its data is non-neutral, and the decoder resets to neutral first and applies only the blocks the frame carries. Sixteen blocks are defined, and `Nfc` at bit 15 exhausts the u16 mask:
...
The router maps the `peer://` path to a target (owner fingerprint, link slot, the advertising connection, and the device id, registered again when the device's slot moves) and sends through `PushOutputEffect` or `PushAudio` on that connection. Dedup varies per channel. Sony effects, scalar vibration, wheel frames, player index, and Guide LED dedup exact repeats. A vibration frame with directional or condition data always ships. The haptic tone dedups only the silent steady state, because an active tone must ship every rumble tick to keep refreshing the owner's hangover expiry.

On the owner, the `FrameReceived` handler `OnRemoteFrameReceived` decodes the frame, resolves its device id to the physical source device, and hands an `Output` frame to `ApplyRemoteOutput`, which takes the sole-writer lease (except for a `PlayerIndex` or `GuideLed` frame, see below), and applies by kind: a Sony effect through `SDL_SendGamepadEffect`, a vibration through `ForceFeedbackState.SetDeviceForces`, a wheel frame through `ApplyRemoteWheel` (which re-encodes with the owner's own vendor writers, see [Wheel Force Feedback Internals](wheel-ffb-internals.md)), a haptic tone (`HapticToneHz`/`HapticToneAmp`) through `HapticToneService.ApplyRemoteTone`, re-encoded by the owner's own per-family writer (Joy-Con HD Rumble, Steam 0x8f, Triton 0x83, Deck), a player index through `SetPlayerIndex` (0-based) for a Nintendo pad (VID `0x057E`), `Ds3DirectService.TrySetPlayerNumber` for a Bluetooth DS3 (`0x054C`/`0x0268`, falling back to `SetPlayerIndex` for a wired one), or `PsMoveDirectService.TrySetPlayerNumber` for a PS Move (`0x054C`/`0x03D5`), a Guide/Home LED brightness through `XboxGipGuideLedWriter.TrySetBrightness` for an Xbox GIP pad, `SteamHomeLedSetter.TrySet` for a 2015 Steam Controller, or `SwitchHomeLedSetter.TrySet` for a Switch HOME-LED pad (the same writers and gate order as the local apply path). An `Audio` frame takes no lease and goes straight to `AudioPassthroughService.FeedRemoteAudio` (see [Controller Audio Internals](controller-audio-internals.md)). The vibration apply has three special cases. A Fanatec pedal re-routes to its raw-HID pedal-rumble writer. A Padix PlayStation converter (#440) records through `TryRecordMotorSnapshot` and writes the converter's own report via `PadixConverterRawHidWriter.Write`. An Xbox One+ impulse-trigger pad, gated by `XboxControllerIdentity.IsImpulseTriggerDevice`, records through `TryRecordXboxImpulseSnapshot` and writes all four channels (large and small plus LT and RT) via `XboxImpulseHidWriter.Write`, with SDL rumble suppressed so the impulse triggers fire. The owner synthesizes nothing here: for the wheel and the haptic tone it is a transport transcoder that re-frames the semantic payload with the writer that owns the real hardware.
...
`InputManager.SetDeviceRestricted` populates the restricted-device set from each peer's gamepad-only flag, set before the device goes online. `IsSlotRestricted(slot)` is true when any restricted device maps to that slot, and it early-outs when no peer is restricted (`RestrictedSnapshot` returns null and the caches rebuild only when the set changes). Five chokepoints honor it:
...
So a gamepad-only peer can drive gamepad slots but never reach the keyboard, mouse, or scroll. `RemoteAssignmentService` applies the same line to remote assignments: a gamepad-only peer can assign its devices only to Xbox, PlayStation, Nintendo, and Extended slots.

---

## Remote devices and disconnect

A `peer://` device relays its owner's real VID and PID, so a linked pad can look like a Bluetooth or Switch 2 controller that the disconnect action would otherwise target. But the disconnecting machine has no radio link to it, so the action could only no-op. `BluetoothLinkHelper.IsDisconnectTarget(devicePath, vendorId, productId, serial)` returns false first for any `peer://` path, before the BthPS3, Switch 2, Joy-Con pair, and Bluetooth-path checks. The macro evaluator uses that gate, so a linked remote device is never a disconnect target (issue #162).
