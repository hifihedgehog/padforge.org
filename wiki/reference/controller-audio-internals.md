...
| File | Role |
|---|---|
| `PadForge.App/Common/Input/AudioPassthroughService.cs` | The core. Sinks, captures, the worker and Bluetooth threads, USB and BT transports, Opus encode, remote-audio hooks, and the composite USB persona feed. `internal static partial class`. |
| `PadForge.App/Common/Input/AudioPassthroughService.PersonaOwnership.cs` | The other half of the partial class: persona feed ownership and retirement, the retired-feed drain, and `CreatePersonaMicCapture` (the USB mic). |
| `PadForge.App/Common/Input/CapturePcmFormat.cs` | The loopback formats a capture accepts. |
| `PadForge.App/Common/Input/DeferredRemoteToneQueue.cs` | Queues a remote-driven tone sink's build off the UDP receive thread. |
...
| `PadForge.Engine/Haptics/TritonPcmEncoder.cs` | (#381) The Steam Controller 2026 PCM stream bytes: the 0x86 command, the 0x88 stereo packet, G.711 mu-law. See [Steam Controller Haptics Internals](steam-controller-haptics-internals.md). |
| `PadForge.App/Common/Input/TritonPcmSupport.cs` | (#381) `TritonPcmLowPassProvider` and the eight-slot `TritonPcmWriteRing`. |
| `PadForge.App/Common/Input/BluetoothLinkHelper.cs` | `TryDisconnect(serial)`, the radio-drop lane the dual-link fix (#387) and Quick Charge (#372) share. |
...
| `PadForge.App/Services/InputService.cs` | Wires the config providers (`PassthroughConfigProvider`, `DspConfigProvider`, and the output-path, buffer-length and macro-demand providers), the router's `SendScopedAudio`, and `OnRemoteFrameReceived`, which hands `Audio` frames to `FeedRemoteAudio`. |

Dependencies: `Concentus` (pure-C# Opus) and `NAudio.Wasapi` (loopback capture, `WasapiOut`, Media Foundation decode), with its `NAudio.Core` dependency supplying the mixers and resamplers. The EQ bands ride `NAudio.Dsp.BiQuadFilter` (RBJ cookbook, already in NAudio.Core). Crossfeed is a C# port of bs2b as vendored in openal-soft's `core/bs2b.cpp` (Boris Mikhaylov, MIT), attributed on all three surfaces.

Identity constants at the top of the service: `Rate = 48000`, `SonyVid = 0x054C`, `Ds5Pids = {0x0CE6, 0x0DF2}` (DualSense, DualSense Edge), `Ds4Pids = {0x05C4, 0x09CC, 0x0BA0}` (DualShock 4 v1, v2, and the Sony USB wireless adapter).
...
`BtWritePool` is a fixed 8-slot overlapped non-blocking HID write pool (`WriteFile` with `FILE_FLAG_OVERLAPPED`). `TrySend` returns immediately. On saturation the DualSense lane drops the frame and the DualShock 4 lane keeps its SBC frames queued for the next tick. An I/O error flags a hard fail. The kernel HID IRP queue is the jitter buffer, roughly 80 ms in flight across the eight slots.
...
1. **`WorkerThreadMain`** ("PadForge.AudioWorker") owns sink I/O. It waits on `_workSignal` or wakes every five seconds and runs `ReconcileOnWorker`. Sink build and teardown, loopback capture start and stop, and the `CreateFile` for each sink's Bluetooth write handle happen here. Two exceptions: `Shutdown` disposes on its caller's thread, and the read-only jack and mic readers (`PadForge.JackWatch`, `PersonaBtMic`, `PadForge.PersonaUsbJack`, `VoiceBtMic`) open their own synchronous handles on their own threads.
...
The worker reconcile runs five phases to keep I/O outside the lock: build the desired-sink list (phase 1, no I/O, taking `_lock` only to re-arm the #387 drop for a Bluetooth-pathed record), diff against `_sinks` (phase 2, under lock, no I/O), build and dispose transports (phase 3, unlocked), reconcile captures (phase 4), then notify `SoundMacroService` and sweep expired vendor-audio tests (phase 5). The desired-sink list walks the 16 slots through `EnumerateAssignedSonyPads` and `FindOnlineSonyDevice`, which gates on `IsOnline`, a non-empty `DevicePath`, the Sony VID, and a DualSense or DualShock 4 PID. Bluetooth is detected by the HID-over-BT service GUID in the device path. A wired DualShock 4 is skipped because it has no USB audio interface. The Sony USB wireless adapter (`0x0BA0`) is the one exception and keeps its sink.
...
`StartCaptureEntry` opens the device and rejects a non-active endpoint. `CreateCaptureEntry` then converts each `DataAvailable` buffer to 48 kHz stereo float in the ring, from any rate and from integer PCM (8, 16, 24 or 32-bit) or float (32 or 64-bit). `CapturePcmFormat` refuses any other format. `SinkSource.Read` holds a per-sink cursor over the shared ring, resyncs only on a genuine stall, and adds the loopback samples on top of the macro mix at full scale. Volume lives in the firmware byte, not the samples.
...
The USB branch of `BuildTransportOnWorker` first drops a stale Bluetooth link (#387, below), then reads the HID interface's PnP container ID through cfgmgr32 and matches a render `MMDevice` whose container ID is the same. If that endpoint is disabled in Windows it re-enables it through `IPolicyConfig`, then restores the prior default if Windows promoted the pad to default. `UsbFrameProvider.Read` reads the device's Audio Output Path once per call and shapes each frame through `MapMirrorChannels`. Default (enum `AudioOutputPath.Automatic`) and Speaker Only silence channel 0 and put the mono program mix `(L+R) * 0.5` on channel 1 (the firmware speaker tap). Headphones (Stereo) puts L on channel 0 and R on channel 1. The mono headset paths put the downmix on channel 0, with channel 1 either silent or a copy for the split path. Follow Headphone Jack resolves to one of the others in `ResolveOutputPath` before the shaper sees it. Channels 2 and 3 are the pad's voice-coil actuators: zero unless a composite persona is feeding them. Playback is `WasapiOut` in shared mode with event sync at 30 ms latency.

### The dual-link drop (#387)

A DualSense that holds Bluetooth and USB at once mutes its USB audio interface in both directions in firmware. This was instrument-confirmed on the owner's pad with PadForge bypassed: a tone played straight into the pad's Windows render endpoint was silent in dual mode and measured 0.50 amplitude on the pad's own microphone in USB-only mode, and the pad's capture endpoint delivered zero buffers (not silence) while dual. The endpoint enumerates Active and `WasapiOut` plays without error, so no counter shows the mute. SDL's USB init de-dups the Bluetooth joystick and moves input to USB, but leaves the radio link up. That lingering link is the mute.

`TryDropStaleBtLinkOnce(sink)` runs at the top of the USB branch, before endpoint resolution. Two gates: the record's serial must parse as a nonzero MAC (an all-zero MAC parses, the #372 lesson), and `DualDropWanted(guid, path, map)` must return true, which it does exactly once per (device, wired path) under `_dualDropLock`. The drop itself is `Task.Run(BluetoothLinkHelper.TryDisconnect(serial))`, the hardware-proven #162 idle-disconnect lane. The reconcile removes the device's map entry whenever its record is Bluetooth-pathed, so unplug, Bluetooth session, replug earns a fresh drop, while a deliberate re-link with the cable still in is never fought. A pad that was never on Bluetooth makes this a radio query that finds nothing. A wired DualShock 4 never reaches the site, because it has no USB audio interface and its sink never builds. The diagnostic line is `SINK dual-link ... dropping stale radio`. `DualLinkAudioDropTests` pins the edge decision and the site.

Verified end to end on hardware: the dual state formed live, the drop fired in 5 s, SDL removed the Bluetooth device, and the mirror measured 0.49 on the pad mic against a 0.50 USB-only baseline. Residuals on record: DualShock 4 dual mode is untested and its fix would run the opposite way (its audio is Bluetooth-only), and `BluetoothSetServiceState` disable-then-enable destroys the pairing, so it is never used as a reconnect lever.
...
`Ds4SbcEncoder` is a clean-room SBC encoder written from the A2DP specification Appendix B, with no libsbc or GPL lineage. Fixed config is 32 kHz, 8 subbands, 16 blocks, joint stereo, SNR allocation, bitpool 48, producing 109-byte frames. The spec's segment-folded analysis window is un-folded at the analysis matrix, and the filterbank conventions were pinned against ffmpeg's decoder at 76.5 and 66.9 dB round-trip SNR.
...
A slot whose virtual controller is a composite USB persona has a real Windows endpoint the game renders into. HIDMaestro delivers that PCM on its pacing thread, in four-channel 48 kHz windows (speaker L/R, haptic L/R) for the DualSense personas and two-channel 32 kHz headset windows for the DualShock 4 persona. `RegisterPersonaFeed` subscribes, and `RoutePersonaFrames` routes each window to the slot's physical Sony pads, resampling the 32 kHz persona to 48 kHz. The feed's presence is itself sink demand, so a composite slot builds its pads' transports with passthrough off and no macros.
...
- `HapticToneReducer` (`PadForge.Engine/Haptics/HapticToneReducer.cs`) reads the dominant frequency from the first normalized-autocorrelation peak above the voiced threshold (not the global maximum) over the playable lag range, and the amplitude from windowed RMS. Below a silence RMS the amplitude is 0. An unvoiced burst holds the last pitch so the coil does not jump around.
...
| Steam Controller 2026 (Triton), Bluetooth `0x1303` | `0x83` LFO-tone OUTPUT report, 10 bytes | `EncodeTritonTone`: frequency `u16` LE plus a signed `gain_db` byte (0 dB unity, floored at -40). Never the 2015 `0x8F` feature report. Grips are driven through a per-note map so they match trackpad pitch. |
| Steam Controller 2026, USB `0x1302` and dongle `0x1304` / `0x1305` | `0x86` configure plus `0x88` stereo PCM, 64 bytes | (#381) Not a tone at all. The sink reads the stereo mix through a Butterworth low-pass and a sinc downsample to 8 kHz and streams it, s16 wired and G.711 mu-law over the dongle. Cut and Fold are not applied. Falls back to `0x83` when the write ring cannot build. Full detail on [Steam Controller Haptics Internals](steam-controller-haptics-internals.md). |
...
The filter runs only on the tone branch. A PCM-capable Triton sink never calls it, since the stream's low-pass cutoff replaces its purpose. `ApplyToneFilter` clamps the user limit to `100..1300` Hz and carries a boundary-hysteresis latch (a Schmitt trigger). The reducer measures pitch on the integer-lag grid `ReduceRate / lag`, so a tone near the limit lands on one of two adjacent grid pitches, and per-tick noise flips which one wins. A bare `>` turns that flip into an octave flap at the tick rate. Once engaged, the tie's lower grid member stays engaged, so the pair folds together instead of alternating folded and unfolded. Disengage needs the measurement a full grid step below the limit. The filter runs on the Test button too, so the test never lies about the setting. Owner-side Remote Link frames are exempt, because the consumer applied its own filter before shipping.

### System-audio mirror and engage gate (#185)

Like the Sony sink, the Wii and haptic-tone sinks carry their own system-audio mirror. `WiiSpeakerService.ReconcileMirrors` and `HapticToneService.ReconcileMirrors` read the same `PassthroughConfigProvider` and add a `WasapiLoopbackCapture` onto the sink's `MacroMixer`, so a mirrored Windows output plays through the Wii speaker, or through a haptic pad reduced to a pitch-following tone (a wired or dongle Steam Controller 2026 streams it as low-passed PCM instead).

The mirror's `BufferedWaveProvider` is 500 ms deep with `DiscardOnBufferOverflow`, and the persona haptic feed's is 250 ms. Both are filled in wall-clock time by other threads and read by the sink thread, so at idle the sink's coarse 15 ms sleep let them fill to cap and every onset queued behind stale audio (#371 follow-up). The idle catch-up drain in `StreamLoop` holds both at one tick of depth. The mechanism is described with the PCM lane on [Steam Controller Haptics Internals](steam-controller-haptics-internals.md), and it runs on every haptic sink that has a persona feed or a mirror, tone sinks included.
...
The Wii Remote has a real speaker, not haptic coils, so its macro-audio output is genuine 8-bit PCM at 2 kHz, not a reduced tone. It is driven by `WiiSpeakerService` (`PadForge.App/Common/Input/WiiSpeakerService.cs`) on its own raw HID handle, following the same per-slot sink model and `MacroMixer` handoff to `SoundMacroService` as the Sony path here and the tone path above. The full protocol lives in [Wii Controllers Internals](wii-controllers-internals.md). It is listed here because it is one of the three macro-audio sink families `SoundMacroService.StartPlacements` fans into, in the order Sony, Wii, tone, and it shares the sink model. Its wire format is PCM speaker reports, distinct from the `HapticToneEncoder` tone bytes.

---

## Remote Link audio

On the receive (owner) side, `InputService` handles the link server's `FrameReceived` event in `OnRemoteFrameReceived`, resolves the frame's device id to the exposed local source device, and calls `AudioPassthroughService.FeedRemoteAudio` for an `Audio` frame. That writes the incoming s16 48 kHz stereo PCM into a `RemoteAudioRing` and marks the device's remote-audio demand. The worker then builds a real Bluetooth or USB sink for it, and `SinkSource.Read` drains the ring instead of loopback and macros, so the relayed audio reaches the physical pad speaker.
...
- [Steam Controller Haptics Internals](steam-controller-haptics-internals.md): the Steam Controller 2026 PCM stream and the 0x83 tone lane.
