...
One more gap is decided in the SDL fork. Its Xbox Elite paddle reader (`SDL_XINPUT_PADDLES`) builds for x64 and, since fork commit a1416320e2, for ARM64. It has two routes. The Bluetooth route uses public WinRT calls and runs on both. The USB and Xbox Wireless Adapter route reads an undocumented format from the Windows GameInput service, so the fork switches it on only where five Windows files match a profile it has checked, by size and SHA-256. The bundled build checks one profile, an x64 one: the four inbox files of the Windows 11 24H2 and 25H2 updates dated 2026-08-27 to 2026-09-14, with the `GameInputRedist.dll` of GameInput redistributable 3.3.221.0. A PC with any other five files reads Elite paddles over Bluetooth only. That covers every ARM64 installation, and on x64 it covers Windows 10, older Windows 11 and other update levels. Measuring an ARM64 profile takes no ARM64 PC, because the files come from Microsoft's symbol server and the GameInput redistributable. hifihedgehog/SDL#32 carries that profile and asks for a check that outlives a Windows update.
